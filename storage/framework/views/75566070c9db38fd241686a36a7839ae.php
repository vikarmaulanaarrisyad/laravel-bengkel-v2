...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#provinces').on('change', function() {
            ...
            $('#districts').html('<option selected disabled value="">Pilih Kecamatan</option>'); // Reset kecamatan
            $('#service').html('<option selected disabled value="">-- Pilih Layanan --</option>'); // Reset layanan
            updateTotal(); // Reset ongkir
        ...
        // Cek ongkir hanya jika KECAMATAN atau KURIR berubah.
        $('#districts, #courier').on('change', function() {
            // Selalu reset layanan dan total ongkir setiap ada perubahan
            $('#service').html('<option selected disabled value="">-- Pilih Layanan --</option>');
            updateTotal(); // Fungsi ini akan mereset ongkir ke 0
            ...
            // Guard Clause: HENTIKAN proses jika salah satu dari kecamatan atau kurir belum dipilih.
            if (!courier || !destination_id) {
                return;
            }

            // Jika lolos, berarti semua data sudah ada. Lanjutkan ke AJAX.
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    if (res.status === 'success' && Array.isArray(res.data) && res.data.length > 0) {
                        res.data.forEach(function (kurir) {
                            kurir.costs.forEach(function (layanan) {
                                let harga = layanan.cost[0].value;
                                let etd = layanan.cost[0].etd;
                                let text = `${kurir.code.toUpperCase()} - ${layanan.service} (${formatRupiah(harga, 'Rp. ')}) (ETD: ${etd} hari)`;

                                $('#service').append(
                                    `<option value="${layanan.service}" data-harga="${harga}">${text}</option>`
                                );
                            });
                        });
                    } else {
        ...
        // Update total ketika layanan pengiriman dipilih
        ...
        function updateTotal(shippingCost = 0) {
            ...
            // Simpan di hidden input untuk form submission
